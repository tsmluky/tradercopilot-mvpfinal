...
class RSIDivergenceStrategy(Strategy):
    ...
    def analyze(self, df: pd.DataFrame, token: str, timeframe: str) -> List[Signal]:
        ...
        # Detectar divergencia bajista (SHORT) - ELIF para mutua exclusividad
        bearish_div = self._detect_bearish_divergence(d, price_highs, rsi_highs)
        # Use ELIF only if bullish wasn't found (or prioritize one)
        # However, to be cleaner, we can check if signals is empty.
        # But `elif bearish_div and not signals:` is clearer.
        elif bearish_div:
            close = float(d['close'].iloc[-1])
            atr = float(d['atr'].iloc[-1]) if not pd.isna(d['atr'].iloc[-1]) else close * 0.02
