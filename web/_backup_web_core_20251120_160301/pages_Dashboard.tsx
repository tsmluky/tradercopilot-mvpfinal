...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Logs evaluados por token (asumimos que fetchLogs devuelve LogsResponse con .logs)
        const ethRes = (await api.fetchLogs(
          AnalysisMode.EVALUATED,
          'eth',
        )) as LogsResponse;
        const btcRes = (await api.fetchLogs(
          AnalysisMode.EVALUATED,
          'btc',
        )) as LogsResponse;
        const solRes = (await api.fetchLogs(
          AnalysisMode.EVALUATED,
          'sol',
        )) as LogsResponse;

        const ethLogs = ethRes.logs ?? [];
        const btcLogs = btcRes.logs ?? [];
        const solLogs = solRes.logs ?? [];

        const allLogs = [...ethLogs, ...btcLogs, ...solLogs];

        // Ordenar por fecha evaluada desc
        allLogs.sort(
          (a, b) =>
        ...
        const last24hLogs = allLogs.filter(
          (l) =>
            new Date(l.evaluated_at || l.timestamp || 0) > oneDayAgo,
        );
        ...
        setWinRate(
          totalClosed24h > 0
            ? Math.round((wins24h / totalClosed24h) * 100)
            : 0,
        );

        // Activas (mock: result === 'OPEN')
        setActiveSignals(
          allLogs.filter((l) => l.result === 'OPEN').length,
        );

        // Evaluadas últimos 7 días
        const last7dLogs = allLogs.filter(
          (l) =>
            new Date(l.evaluated_at || l.timestamp || 0) > sevenDaysAgo,
        );
        setEvaluatedCount(last7dLogs.length);

        // PnL estimado en R (mock)
        let pnl = 0;
        last7dLogs.forEach((l) => {
          if (l.result === 'WIN') pnl += 1.5; // aprox 1.5R por win
          if (l.result === 'LOSS') pnl -= 1.0; // -1R por loss
        });
        setEstPnL(parseFloat(pnl.toFixed(1)));

        // Construcción datos del gráfico (últimos 7 días)
        ...
        last7dLogs.forEach((l) => {
          const d = new Date(l.evaluated_at || l.timestamp || 0);
          const key = d.toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
          });
          if (!dailyStats[key]) return;
          if (l.result === 'WIN') dailyStats[key].wins++;
          if (l.result === 'LOSS') dailyStats[key].losses++;
        });
        ...
        <div className="mb-8 bg-gradient-to-r from-indigo-900/40 to-slate-900 rounded-2xl border border-indigo-500/30 p-6 md:p-8 shadow-xl relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-slate-300 max-w-xl mb-6">
              Your terminal is ready. The AI is online and scanning the
              markets. Initialize your first scan to start building your
              portfolio.
            </p>
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-bold text-white text-sm">
                      {log.token?.toUpperCase() ?? '—'}
                    </span>
                    <span className="text-[10px] text-slate-500 font-mono">
                      {new Date(
                        log.evaluated_at || log.timestamp || '',
                      ).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
