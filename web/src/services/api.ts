...
async function sendAdvisorChat(
  ...
  // Si es el mensaje inicial (historial vacío), usamos el análisis estándar
  if (!history.length) {
  ...
  // Para mensajes siguientes, llamamos al endpoint de chat real
  try {
    const body = {
      history: history.map(m => ({ role: m.role, content: m.content })), // Enviamos solo lo necesario
      context: context
    };

    const res = await fetchWithTimeout(`${API_BASE_URL}/analyze/advisor/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
